  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (esZip) {
        ...
        const omitidosComplementoPago = data.omitidos_complemento_pago ?? 0;
        const noXml = data.no_xml ?? 0;
        const archivosNoXml = Array.isArray(data.archivos_no_xml) ? data.archivos_no_xml : [];
        const errores = data.errores ?? 0;
        setZipResumen({ exitos, duplicados, mensaje: data.mensaje, detalles: data.detalles ?? [] });

        if (exitos === 0) {
          const detalles = Array.isArray(data.detalles) ? data.detalles : [];
          const archivosConError = detalles
            .filter((detalle) => detalle?.status === 'error')
            .slice(0, 3)
            .map((detalle) => detalle?.archivo)
            .filter(Boolean);

          if (duplicados > 0 && errores === 0 && omitidosComplementoPago === 0) {
            setErrorMsg('No se guardo nada nuevo: todos los XML del ZIP ya estaban registrados (UUID duplicados).');
          } else if (noXml > 0 && errores === 0 && duplicados === 0 && omitidosComplementoPago === 0) {
            const muestraNoXml = archivosNoXml.slice(0, 5).join(', ');
            const detalleNoXml = muestraNoXml ? ` Archivos no XML: ${muestraNoXml}.` : '';
            setErrorMsg(`ZIP invalido para CFDI: contiene ${noXml} archivo(s) que no son XML.${detalleNoXml}`);
          } else if (errores > 0) {
            const listaErrores = archivosConError.length > 0
              ? ` Archivos con error: ${archivosConError.join(', ')}.`
              : '';
            setErrorMsg(`No se guardo nada nuevo: se detectaron ${errores} XML con error o vacios.${listaErrores}`);
          } else if (omitidosComplementoPago > 0) {
            setErrorMsg('No se guardo nada nuevo: los XML del ZIP corresponden a complementos de pago y fueron omitidos.');
          } else {
            setErrorMsg(data.mensaje || 'No se importo ningun XML nuevo.');
          }
          setStatus('error');
          return;
        }
        ...
        <div className="p-8">
          ...
          {status === 'success' && (
            <div className="mt-4 flex items-center gap-2 text-green-400 bg-green-400/10 p-3 rounded-xl text-sm">
              <CheckCircle2 className="w-4 h-4" /> Procesado con exito
            </div>
          )}
